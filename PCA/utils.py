...
def generate_simulation_data(
    n_classes=2, n_samples=[20, 20], n_features=3, seed=0, scales=None
):
    # initial checks
    ...
    assert n_classes == len(
        n_samples
    ), "n_samples has to be an array with as many elements as n_classes"
    if sp.any(scales) == None:
        scales = sp.ones(n_features)
    else:
        assert (
            len(scales) == n_features
        ), "scales has to be an array with as many elements as features"
    ...
    for i in range(n_classes):
        ...
        class_matrix = sp.random.multivariate_normal(
            sp.ones(n_features) * (i + 1) * sp.random.randn(n_features),
            sp.eye(n_features),
            n_samples[i],
        )
        # store data
        class_labels = (
            sp.ones(n_samples[i]) * (i + 1)
            if (sp.any(class_labels) == None)
            else sp.concatenate([class_labels, sp.ones(n_samples[i]) * (i + 1)])
        )
        feature_matrix = (
            class_matrix
            if (sp.any(feature_matrix) == None)
            else sp.vstack([feature_matrix, class_matrix])
        )
    # scale features
    if sp.any(scales) != None:
